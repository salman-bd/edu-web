...
export default function TeacherProfileForm({ dbData }: TeacherProfileFormProps) {  
  ...
  const isAffiliated = csc? true : dbData?.isAffiliated || false;  

  // console.log("CSC, dbData.isAffiliated, isAffiliated: ", csc, dbData?.isAffiliated, isAffiliated);
  
  const form = useForm<z.infer<typeof teacherProfileSchema>>({  
    resolver: zodResolver(teacherProfileSchema),  
    defaultValues: {  
      avatar: dbData?.avatar || undefined,  
      name: dbData?.name || '',  
      institutionName: dbData?.institutionName || csc || '',  
      designation: dbData?.designation || '',  
      email: dbData?.email || '',  
      contactNo: dbData?.contactNo || '',  
      school: dbData?.school || '',  
      sscPassingYear: dbData?.sscPassingYear || '',  
      college: dbData?.college || '',  
      hscPassingYear: dbData?.hscPassingYear || '',  
      university: dbData?.university || '',  
      graduationYear: dbData?.graduationYear || '',  
